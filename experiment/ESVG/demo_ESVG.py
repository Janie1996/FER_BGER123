...
from Code.RNN import attention_2D_2dim, D2_emotion
from Code.model import resnet18
from Code.load_dataset import LoadData
from Code.load_parameter import savecheckpoint
#from Code.multiview_2Dconv_model import resnet18
#from Code.RNN.tt import resnet18
import torchvision.models as models
from Code.model.new_cnn_model import Cost, costblock
#from Code.RNN.D2_emotion import resnet18
# from Code.RNN.attention_2D_2dim import resnet18
import numpy as np
...
def main(times):
    ...
    best_pre = 0
    ''' Load data '''
    # dir_train = 'E:/ESVG/emotional database/phase_1/test'
    # list_train = 'E:/PycharmProjects/facial expression/experiment/ESVG/data/ESVG_train'+str(times)+'.txt'
    # batch_train=8
    #
    # dir_eval = 'E:/ESVG/emotional database/phase_1/test'
    # list_eval = 'E:/PycharmProjects/facial expression/experiment/ESVG/data/ESVG_eval'+str(times)+'.txt'
    # batch_eval=16
    ...
    model = resnet18.resnet18(batch=batch_train, C=args.C, K=args.K, num_classes=7)
    #model = resnet18(batch=batch_train, C=args.C, num_classes=7)
    # args1 = pretrainedmodels.__dict__['resnet18'](pretrained='imagenet').state_dict()
    model_state_dict = model.state_dict()
    args1 = torch.load('./ESVG_model/ESVG_resnet18_CELoss2_0.989')['state_dict']
    for key in args1:
    ...
    #model = Cost(7, costblock, [2, 2, 2, 2])
    ...
    for epoch in range(args.epochs):
        ...
        prec1, prec2, prec3, prec4,p1,p2,p3,p4,p5 = validate(val_loader, model)


        prec1 = max(prec1,prec2,prec3,prec4,p1,p2,p3,p4,p5)
    ...
    savecheckpoint({
        ...
        'prec1': best_pre,
    },'E:/PycharmProjects/facial expression/experiment/ESVG/model/ESVG_resnet18_CELoss'+str(times))

def validate1(val_loader,model):
    ...
    with torch.no_grad():
        for i, (video, index) in enumerate(val_loader):
            ...
            if (i == 0):
                endpre = p3.data.cpu().numpy()
            else:
                endpre = np.vstack((endpre, p3.data.cpu().numpy()))
            ...
            # p1,p2,p3,p4,p5=fusion(p1,p2,p3,target.data)
            # corrects = float(torch.sum(torch.from_numpy(p1) == target.cpu().data)) / float(target.size(0))
            # end.update(corrects, target.size(0))
            #
            # corrects = float(torch.sum(torch.from_numpy(np.array(p2)) == target.cpu().data)) / float(target.size(0))
            # end1.update(corrects, target.size(0))
            #
            # corrects = sum(p3==target.cpu().data.numpy())/float(target.size(0))
            # end2.update(corrects, target.size(0))
            #
            # corrects = sum(p4==target.cpu().data.numpy())/float(target.size(0))
            # end3.update(corrects, target.size(0))
            #
            # corrects = sum(p5==target.cpu().data.numpy())/float(target.size(0))
            # end4.update(corrects, target.size(0))
    ...
    pre = np.load("E:/PycharmProjects/facial expression/experiment/ESVG/pre.npy")
    target = np.load('E:/PycharmProjects/facial expression/experiment/ESVG/target.npy')
    ...
    print('score {end.avg:.3f}   score_max {end1.avg:.3f}   decision {end2.avg:.3f}   decision {end3.avg:.3f}   decision {end4.avg:.3f}'.format(end=end, end1=end1, end2=end2, end3=end3, end4=end4))

    return topVideo.avg,pred_p1.avg,pred_p2.avg,pred_p3.avg,end.avg,end1.avg,end2.avg,end3.avg,end4.avg
