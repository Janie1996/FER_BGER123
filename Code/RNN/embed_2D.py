...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out=clips.permute(1, 2, 0, 3, 4).cuda()
        del clips
...
class ResNet(nn.Module):
    ...
    def forward(self, x):  # [batch*clips, chanel, frame,h,w]
        ...
        x = input.view(-1, 3, 224, 224).cuda()   #[batch*clips*frame,channel,h,w]
        del input
        #
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)   #  #[batch*clips*frame,channel,h,w]
