...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)    # batch*length,feature

        batch=int(x.shape[0]/self.C)
        clips = torch.ones([self.C,batch , self.rnn_size])
        for i in range(self.C):
            clips[i] = x[i * batch:(i + 1) *batch, :]
        clips = clips.transpose(0, 1).cuda()     # batch,length,feature

        end = self.rnn(clips)   # batch,7

        out=self.dropout(x)
        out=self.fc1(out)    # batch*length,7
        ...
        clips = clips.transpose(0, 1).cuda()     # batch,length,7
